...
- `soothe_deepagents.provider_profiles` — plugins that call
    `register_provider_profile(...)` to declare provider- or model-keyed
    `ProviderProfile` entries.
- `soothe_deepagents.harness_profiles` — plugins that call
...
from soothe_deepagents.profiles.harness import (
...
from soothe_deepagents.profiles.provider import _nvidia, _openai, _openrouter
...
_PROVIDER_PROFILE_GROUP = "soothe_deepagents.provider_profiles"
"""Entry-point group name for third-party `ProviderProfile` plugins."""

_HARNESS_PROFILE_GROUP = "soothe_deepagents.harness_profiles"
...
def _ensure_builtin_profiles_loaded() -> None:
    ...
    1. Call the built-in provider `register` functions directly.
        Any exception propagates — a broken built-in is a soothe_deepagents
        bug and must surface loudly, not degrade silently.
    2. Iterate `importlib.metadata` entry points in the
        `soothe_deepagents.provider_profiles` and `soothe_deepagents.harness_profiles`
        groups. Third-party failures are logged at `WARNING` and skipped
        so one misbehaving distribution cannot prevent `soothe_deepagents.profiles`
        from importing.
    ...
    `get_*_profile` entry points; importing `soothe_deepagents.profiles` itself
...
def _invoke_profile_plugins(group: str) -> None:
    ...
    Args:
        group: Entry-point group name to iterate (e.g.
            `soothe_deepagents.provider_profiles`).
    """
    try:
