"""Tests for soothe_deepagents._models helpers and internal profile registries."""
...
from soothe_deepagents._models import (
...
from soothe_deepagents.profiles import (
...
from soothe_deepagents.profiles.provider._nvidia import (
    _NVIDIA_APP_ORIGIN,
    _NVIDIA_BILLING_ORIGIN_HEADER,
    _nvidia_attribution_kwargs,
)
from soothe_deepagents.profiles.provider._openrouter import (
...
class TestResolveModel:
    ...
    def test_openai_prefix_uses_responses_api(self) -> None:
        with patch("soothe_deepagents._models.init_chat_model") as mock:
    ...
    def test_openrouter_env_var_overrides_app_url(self) -> None:
        ...
        with (
            patch("soothe_deepagents._models.init_chat_model") as mock,
...
class TestCheckOpenRouterVersion:
    ...
    def test_passes_when_not_installed(self) -> None:
        with patch(
            "soothe_deepagents.profiles.provider._openrouter.pkg_version",
    ...
    def test_passes_when_version_above_minimum(self) -> None:
        with patch("soothe_deepagents.profiles.provider._openrouter.pkg_version", return_value="99.0.0"):
            check_openrouter_version()

    def test_raises_when_version_too_old(self) -> None:
        with (
            patch("soothe_deepagents.profiles.provider._openrouter.pkg_version", return_value="0.0.1"),
            pytest.raises(ImportError, match="langchain-openrouter>="),
        ):
            check_openrouter_version()

    def test_skips_check_for_invalid_version(self) -> None:
        with patch("soothe_deepagents.profiles.provider._openrouter.pkg_version", return_value="not-a-version"):
            check_openrouter_version()

    def test_resolve_model_calls_check(self) -> None:
        with (
            patch("soothe_deepagents.profiles.provider._openrouter.check_openrouter_version") as mock_check,
            patch("soothe_deepagents._models.init_chat_model") as mock_init,
        ):
...
class TestProfilePluginLoader:
    ...
    def _isolate_loader_state(self) -> Iterator[None]:
        ...
        """
        from soothe_deepagents.profiles import _builtin_profiles  # noqa: PLC0415
    ...
    def test_iterates_both_entry_point_groups(self) -> None:
        ...
        from soothe_deepagents.profiles._builtin_profiles import (  # noqa: PLC0415
        ...
        with patch(
            "soothe_deepagents.profiles._builtin_profiles.entry_points",
            return_value=[],
        ) as mock:
            _ensure_builtin_profiles_loaded()
    ...
    def test_broken_plugin_logged_and_skipped(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        def fake_entry_points(*, group: str) -> list[MagicMock]:
            if group == "soothe_deepagents.provider_profiles":
                return [broken, good]
            return []

        with (
            caplog.at_level(logging.ERROR, logger="soothe_deepagents.profiles._builtin_profiles"),
    ...
    def test_entry_points_call_itself_raises(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with (
            caplog.at_level(logging.WARNING, logger="soothe_deepagents.profiles._builtin_profiles"),
    ...
    def test_bootstrap_harness_keys_snapshot_after_load(self) -> None:
        ...
        def fake_entry_points(*, group: str) -> list[MagicMock]:
            if group == "soothe_deepagents.harness_profiles":
                return [ep]
            return []
    ...
    def test_bootstrap_failure_rolls_back_and_waiter_retries(
        ...
        from soothe_deepagents.profiles import _builtin_profiles  # noqa: PLC0415
        from soothe_deepagents.profiles.provider import _openrouter  # noqa: PLC0415
        ...
        with (
            caplog.at_level(logging.INFO, logger="soothe_deepagents.profiles.provider.provider_profiles"),
            caplog.at_level(logging.ERROR, logger="soothe_deepagents.profiles._builtin_profiles"),
            patch("soothe_deepagents.profiles._builtin_profiles.entry_points", return_value=[]),
    ...
    def test_concurrent_first_read_waits_for_bootstrap(self) -> None:
        ...
        from soothe_deepagents.profiles import _builtin_profiles  # noqa: PLC0415
        from soothe_deepagents.profiles.provider import _openai  # noqa: PLC0415
    ...
    def test_concurrent_user_registration_waits_until_snapshot_taken(self) -> None:
        ...
        from soothe_deepagents.profiles import _builtin_profiles  # noqa: PLC0415
        from soothe_deepagents.profiles.harness import _anthropic_opus_4_7  # noqa: PLC0415
        from soothe_deepagents.profiles.harness.harness_profiles import _has_any_harness_profile  # noqa: PLC0415
...
class TestLazyBootstrap:
    ...
    `soothe_deepagents.profiles` import to keep cold-importing
    `soothe_deepagents._models` (and therefore `deepagents_cli` startup) cheap
    ...
    def test_importing_models_does_not_bootstrap(self) -> None:
        """Cold-importing `soothe_deepagents._models` must not trigger bootstrap."""
        out = self._run(
            "import soothe_deepagents._models\n"
            "from soothe_deepagents.profiles import _builtin_profiles\n"
            "from soothe_deepagents.profiles.harness.harness_profiles import _HARNESS_PROFILES\n"
            "from soothe_deepagents.profiles.provider.provider_profiles import _PROVIDER_PROFILES\n"
    ...
    def test_model_matches_spec_does_not_bootstrap(self) -> None:
        ...
        out = self._run(
            "from soothe_deepagents._models import model_matches_spec\n"
            "from soothe_deepagents.profiles import _builtin_profiles\n"
            "from soothe_deepagents.profiles.harness.harness_profiles import _HARNESS_PROFILES\n"
            "from soothe_deepagents.profiles.provider.provider_profiles import _PROVIDER_PROFILES\n"
            "print(_builtin_profiles._loaded, len(_PROVIDER_PROFILES), len(_HARNESS_PROFILES))\n"
    ...
    def test_get_provider_profile_triggers_bootstrap(self) -> None:
        ...
        out = self._run(
            "from soothe_deepagents.profiles.provider.provider_profiles import get_provider_profile\n"
            "from soothe_deepagents.profiles import _builtin_profiles\n"
    ...
    def test_register_harness_profile_triggers_bootstrap(self) -> None:
        ...
        out = self._run(
            "from soothe_deepagents.profiles.harness.harness_profiles import register_harness_profile\n"
            "from soothe_deepagents import HarnessProfile\n"
            "from soothe_deepagents.profiles import _builtin_profiles\n"
            "before = _builtin_profiles._loaded\n"
...
class TestProfileLookupBreadcrumb:
    ...
    def test_harness_exact_miss_logs_breadcrumb(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        try:
            ...
            with caplog.at_level(logging.DEBUG, logger="soothe_deepagents.profiles.harness.harness_profiles"):
    ...
    def test_provider_exact_miss_logs_breadcrumb(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        try:
            ...
            with caplog.at_level(logging.DEBUG, logger="soothe_deepagents.profiles.provider.provider_profiles"):
...
class TestGetModelProviderLogging:
    ...
    def test_logs_exception_at_info_with_module_qualname(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="soothe_deepagents._models"):
            assert get_model_provider(model) is None
...
class TestChainedPreInitAndFactoryErrorLogging:
    ...
    def test_chained_pre_init_logs_base_failure(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        try:
            ...
            assert merged.pre_init is not None
            with (
                caplog.at_level(logging.ERROR, logger="soothe_deepagents.profiles.provider.provider_profiles"),
