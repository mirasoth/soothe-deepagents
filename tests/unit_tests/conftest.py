...
from soothe_deepagents._api.deprecation import reset_deprecation_dedupe
...
def _discover_deprecated_targets() -> tuple[object, ...]:
    """Walk the `soothe_deepagents` package for `@deprecated`-wrapped callables.
    ...
    for module_info in pkgutil.walk_packages(soothe_deepagents.__path__, soothe_deepagents.__name__ + "."):
        try:
...
def _reset_video_dep_cache() -> None:
    ...
    """
    from soothe_deepagents.middleware._video import video_dependencies_available  # noqa: PLC0415
