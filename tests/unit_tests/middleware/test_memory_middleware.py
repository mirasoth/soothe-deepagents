...
from soothe_deepagents.middleware.memory import MEMORY_SYSTEM_PROMPT, MemoryMiddleware
...
def test_create_deep_agent_with_memory_default_backend() -> None:
    ...
    agent = create_deep_agent(
        memory=["/user/.soothe_deepagents/AGENTS.md"],
    ...
    assert "/user/.soothe_deepagents/AGENTS.md" in state_values["files"]
    assert "memory_contents" in state_values
    assert "/user/.soothe_deepagents/AGENTS.md" in state_values["memory_contents"]
...
def test_html_comments_stripped_from_memory_before_injection(tmp_path: Path) -> None:
    ...
    (tmp_path / "AGENTS.md").write_text(
        "<!-- soothe_deepagents:onboarding-name:start -->\n"
        '- The user\'s preferred name is "Alice".\n'
        "<!-- soothe_deepagents:onboarding-name:end -->\n"
        "\nSome other memory.\n",
        encoding="utf-8",
    )
    middleware = MemoryMiddleware(
